/**
 * @file MarkersOverlay.qml
 * @brief Generic map markers overlay — custom point-of-interest markers (stub).
 *
 * Stub implementation. Will display server-driven markers sourced from
 * SceneDataPush (modelId="markers"). Calls background.latLonToScreen(lat, lon) to
 * position markers on the active background.
 */
import QtQuick
